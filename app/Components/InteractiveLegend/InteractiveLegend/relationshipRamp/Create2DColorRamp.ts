            ...
            const queryFeatureCount = promiseUtils.debounce(() => {
              ...
              const queryFeatureCount =
                ...
                this.layerView.queryFeatureCount({
                  ...
                  outSpatialReference,
                  where,
                  timeExtent: this.layerView?.filter?.timeExtent
                    ? this.layerView.filter.timeExtent
                    : this.layerView?.effect?.filter?.timeExtent
                    ? this.layerView.effect.filter.timeExtent
                    : null
                });
            ...
            if (!this._handles.has(featureCountKey)) {
              this._handles.add(
                [
                  ...
                  watchUtils.watch(
                    this.layerView,
                    "filter.timeExtent, effect.filter.timeExtent",
                    () => {
                      queryFeatureCount();
                    }
                  )
              ...
              this._handles.add(
                watchUtils.watch(this, "layerView.filter.where", () => {
                  const cellGroup = this.colorRampProperties.surface.children[0]
                    .children;
                  if (!this.layerView?.filter?.where) {
            ...
            watchUtils.when(this, "layerView.effect", () => {
              ...
              this._handles.add(
                watchUtils.watch(this, "layerView.effect.filter.where", () => {
                  const cellGroup = this.colorRampProperties.surface.children[0]
                    .children;
                  if (!this.layerView?.effect?.filter?.where) {
                    cellGroup.forEach(cell => {
                      const { rawNode } = cell;
      ...
      if (this.filterMode === "mute") {
        ...
        this.layerView.effect = new FeatureEffect({
          filter: new FeatureFilter({
            where,
            timeExtent: this.layerView?.effect?.filter?.timeExtent
              ? this.layerView.effect.filter.timeExtent
              : null
          }),
