            ...
            switch (symbol.type) {
                ...
                case SIMPLE_FILL_SYMBOL:
                    if (style === "solid") {
                        ...
                        fill = __assign({}, gfxBase.defaultPattern, { src: FILL_PATTERN_URL_PREFIX + style + ".png", width: 8, height: 8 });
                    }
                    break;
                case PICTURE_FILL_SYMBOL:
                    fill = __assign({}, gfxBase.defaultPattern, { src: symbol.url, width: pt2px(symbol.width) * symbol.xscale, height: pt2px(symbol.height) * symbol.yscale, x: pt2px(symbol.xoffset), y: pt2px(symbol.yoffset) });
                    break;
                case TEXT_SYMBOL:
                    fill = symbol.color;
                    break;
